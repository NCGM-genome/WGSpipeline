...
process fq2cram {
    ...
    script:
    """
    bash /tools/pbrun-fq2bam.sh '$fq1_list' '$fq2_list' '$rg_list' $ref '$bwa_options' $num_gpus $prefix $mode '$knownSites_list'
    """
}
...
workflow {
    ...
    knownSites_list = params.findAll { it.key.startsWith('knownSites_') }
        ...
        .sort()
        .join(",")
    knownSites_list = Channel.of(knownSites_list)
    ...
    // ################ workflow ################
    // #### fq2cram ####
    fq2cram_out = fq2cram(fq1_list, fq2_list, rg_list, ref, bwa_options, prefix, num_gpus, mode, knownSites_list)
