...
workflow {
    // #### fq2cram ch####
    // fastqペアの1要素目を集めたリストをカンマ区切りにする
    fq1_list = Channel.fromFilePairs("${params.fastq_dir}*_{1,2}.fastq")
        .map {it[1][0]}
        .collect()
        .map { it.sort() } 
        .map { it.join(",") }
    // fastqペアの2要素目を集めたリストをカンマ区切りにする
    fq2_list = Channel.fromFilePairs("${params.fastq_dir}*_{1,2}.fastq")
        .map {it[1][1]}
        .collect()
        .map { it.sort() } 
        .map { it.join(",") }
